...
<div class="max-w-6xl mx-auto p-4">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white shadow-md rounded">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-2 text-center text-gray-500">Nema zadataka</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <div class="grid grid-cols-3 gap-4 mb-8">
        @foreach (['pending', 'in_progress', 'completed'] as $status)
            <div
                class="bg-gray-100 p-4 rounded shadow hover:bg-gray-300 transition-colors"
                data-status="{{ $status }}"
                ondragover="allowDrop(event)"
                ondrop="drop(event)"
            >
                <h3 class="font-bold text-lg mb-4">{{ ucfirst(str_replace('_', ' ', $status)) }}</h3>
